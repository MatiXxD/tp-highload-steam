...
| Африка               | 132.91     | 132.91     |
| Центральная Америка  | 183.67     | 183.67     |

P.S. для Южной Америки делил на `2.5`, т.к. часть трафика может обрабатываться серверами Северной Америки.

**Исходя из этого имеет смысл сделать 2 типа ДЦ, одни расчитаны на более тяжелую нагрузку ~1500 RPS, другие на слабую -- ~500 RPS (т.е. будут в 3 раза менее требовательные).**
...
- *Graceful Degradation:* если какая-то часть системы упала, то сервис продолжает функционировать. Вот примеры сценариев:
  ...
  - если у нас вдруг упадет микросервис с информацией по играм (тот, что работает с таблице `GameInfo`), то мы по прежнему сможем отображать базовую информацию по игре, для этого мы можем сходить в микросервис игры (который работает `Game`)
...
### Расчет ресурсов
При расчете будем использовать, что для 100 RPS надо:
- 1 CPU
- 1 ГБ RAM
...
|:--------------------|-------------:|----------:|:--------|
| ReviewService       |     0.32     |      1    | 1 Gb    |
| WalletService       |     11.98    |      1    | 1 Gb    |
| TradeService        |     578.68   |      6    | 6 Gb    |
| AuthService         |     51.60    |      1    | 1 Gb    |
| UserService         |     13628    |      137  | 137 Gb  |
| GameService         |     7030.18  |      71   | 71 Gb   |
| ItemService         |     7986.12  |      80   | 80 Gb   |
| ScreenshotService   |     257.47   |      3    | 3 Gb    |
| OrderService        |     11.98    |      1    | 1 Gb    |
| AchievementService  |     600      |      6    | 6 Gb    |
| MultiplayerService  |     1401.56  |      15   | 15 Gb   |

**Расчет для БД:**
Для диска берется запас (умножение на 1.2)
| БД             | RPS        | CPU        | RAM (ГБ)           | Disk       |
|:---------------|-----------:|-----------:|-------------------:|-----------:|
| MongoDB        | 53614.8    | 537        | 560 + 3*23 = 649   | 38.38 ТБ   |
| Tarantool      | 11.98      | 1          | 45*2 + 1 = 91      | 219 ГБ     |
| Redis          | 1453.16    | 15         | 2*222.4 + 15 = 460 | 530 ГБ     |
| Elasticsearch  | 1350       | 14         | 14 + 0.4*100 = 54  | 240 ГБ     |
| AWS            | 1350       | 0          | 0                  | 51.6 ТБ    |

В данной таблицы учитывается, что для Redis и Tarantool мы храним данные в RAM, также в RAM для MongoDB учитывается размер индексов. Также учитывается добавочный размер на реплики.

### Развертывание сервиса
В качестве модели развертывания будем использовать оркестрацию Docker-контейнеров с помощью kubernetes.

**Распределение контейнеров по сервисам:**
| Сервис              | CPU cores | RAM    | Кол-во контейнеров (2 CPU, 2 ГБ RAM) |
|:--------------------|----------:|:-------|-------------------------------------:|
| ReviewService       | 1          | 1 ГБ   | 1 |
| WalletService       | 1          | 1 ГБ   | 1 |
| TradeService        | 6          | 6 ГБ   | 3 |
| AuthService         | 1          | 1 ГБ   | 1 |
| UserService         | 137        | 137 ГБ | 69 |
| GameService         | 71         | 71 ГБ  | 36 |
| ItemService         | 80         | 80 ГБ  | 40 |
| ScreenshotService   | 3          | 3 ГБ   | 2 |
| OrderService        | 1          | 1 ГБ   | 1 |
| AchievementService  | 6          | 6 ГБ   | 3 |
| MultiplayerService  | 15         | 15 ГБ  | 8 |

### Конфигурации серверов
| Назначение          | Хостинг  | Конфигурация                              | CPU cores | Стоимость за сервер  | Кол-во серверов |
|:--------------------|:---------|:------------------------------------------|-----------|---------------------:|----------------:|
| Kubenode (PodNodes) | own      | 2×6448H / 8×16ГБ / 2×NVMe 4ТБ / 2×25Gb/s  |    32     | $15,182.42           | 10              |
| MongoDB             | own      | 2×5418Y / 8×16ГБ / 2×NVMe 4ТБ / 2×10Gb/s  |    24     | $10,332.20           | 12              |
| Tarantool           | own      | 2×5415+ / 16×16ГБ / 1×NVMe 1ТБ / 2×10Gb/s |    8      | $7,888.75            | 3               |
| Redis               | own      | 2×6426Y / 16×32ГБ / 1×NVMe 2ТБ / 2×25Gb/s |    16     | $10,427.84           | 3               |
| Elasticsearch       | own      | 2×6426Y / 16×32ГБ / 1×NVMe 4ТБ / 2×25Gb/s |    16     | $10,427.84           | 3               |
